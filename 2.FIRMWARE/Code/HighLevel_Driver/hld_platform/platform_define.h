...
#ifndef COMPONENTS_DRIVER_PORTING_PLATFORM_PLATFORM_PORTING_H_
#define COMPONENTS_DRIVER_PORTING_PLATFORM_PLATFORM_PORTING_H_
...
/******************************************************************************/
//  TYPEDEF. MACRO. DEFINE
/******************************************************************************/
/* Porting for status of application base on status of mcu */
typedef enum
{
	/* Status as Success or OK. Base on STATUS_SUCCESS */
	EV_STT_SUCCESS = STATUS_SUCCESS,
	EV_STT_OK = EV_STT_SUCCESS,

	/* Have error or failed. Base on STATUS_ERROR */
	EV_STT_ERROR = STATUS_ERROR,
	EV_STT_FAILED = EV_STT_ERROR,

	/* Is Busy. Base on STATUS_BUSY */
	EV_STT_BUSY = STATUS_BUSY,

	/* Is timeout. Base on STATUS_TIMEOUT */
//	EV_STT_TIMEOUT = STATUS_TIMEOUT,

	/* Have unsupported . Base on STATUS_UNSUPPORTED */
//	EV_STT_UNSUPPORTED = STATUS_UNSUPPORTED,

	/**
	 * Define for local status
	 */
	EV_STT_LOCAL_START = 0xF000,
	/* FIFO Empty */
	EV_STT_FIFO_EMPTY,
	/* FIFO full */
	EV_STT_FIFO_FULL,
...
/*! reserved signals */
enum {
    ENTRY_SIG = 0U,     /*!< signal for coding entry actions */
    EXIT_SIG  = 1U,     /*!< signal for coding exit actions */
    USER_SIG  = 2U      /*!< first signal that can be used for user signals */
};
...
static inline uint32_t __CLZ1(uint32_t data)
{
...
#define PLATFORM_LOG2(n_)           ((uint8_t)(31U - __CLZ1(n_)))
#define PLATFORM_ON_IDLE()          __WFI()

/** Assert porting */
//static inline void __sys_assert(bool x)
//{
//    if(x) { } else {DBG_ERRO("__sys_assert\r\n"); BKPT_ASM; for(;;) {} }
//}
#endif /* COMPONENTS_DRIVER_PORTING_PLATFORM_PLATFORM_PORTING_H_ */
